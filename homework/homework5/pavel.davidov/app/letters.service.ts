...
export class LettersService {
  ...
  getOneById(id: string) {
    return this.http.get<Letter[]>(`${url}${id}`);
  }
  ...
  save(letter: Letter): Observable<Letter> {
    return this.http
      .post<Letter>(url, letter)
      .pipe(tap(_ => this.saveLog(`POST ${url}`)));
  }

  setMailbox(name: string): Observable<Mailbox> {
    return this.http
      .post<Mailbox>(
        'http://test-api.javascript.ru/v1/pdavydov/mailboxes/',
        new Mailbox(name)
      )
      .pipe(tap(_ => this.saveLog(`POST ${url}`)));
  }
  ...
  clear(letter: Letter) {
    return this.http
      .delete(`${url}${letter._id}`, { responseType: 'text' })
